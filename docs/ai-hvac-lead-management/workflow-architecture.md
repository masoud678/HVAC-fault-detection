...
Assignment:
  claudePrompt: |
    ...
    - technical_reason: brief reason
    - recommended_action: what to do
    - response_timeline: timeline
    - suggested_message: message for customer
```
    ...
    (() => {
      try {
        ...
        return {
          ...
          timeline: parsed.response_timeline || "",
          message: parsed.suggested_message || ""
        };
      } catch(e) {
...
Assignments:
  ...
  leadRecord:
    ...
    priority_score: $json.parsedLead.priority_score
...
Configuration:
  ...
  sheetName: "HVAC_LEADS" (gid: 1731831536)
...
Configuration:
  ...
  text: |
    ...
    📊 Priority: {{ $json.leadRecord.priority_score }}/100
    
    🎯 Fault: {{ $json.leadRecord.fault }}
    Next: {{ $json.leadRecord.action }}

Credentials:
  telegramApi: Customer322_bot
```
...
Configuration:
  ...
  message: |
    ...
    AI ASSESSMENT:
    Severity: {{ $json.leadRecord.severity }}
    Priority Score: {{ $json.leadRecord.priority_score }}/100
    Fault: {{ $json.leadRecord.fault }}
...
Assignment:
  response:
    ...
    severity: $json.leadRecord.severity
    priority_score: $json.leadRecord.priority_score
...
4. Still save to sheets (status: "PARSE_ERROR")
```
...
| Claude timeout | Increase max_tokens, reduce temperature, check API status |
...
- [`README.md`](README.md) — Business overview
- [`lead-scoring-criteria.md`](lead-scoring-criteria.md) — Scoring rules
- [`setup-guide.md`](setup-guide.md) — Deployment steps
